...
class Change_Num_Otp extends StatefulWidget {
  final String number;
  const Change_Num_Otp({Key? key, required this.number}) : super(key: key);

  @override
  _Change_Num_OtpState createState() => _Change_Num_OtpState();
}

class _Change_Num_OtpState extends State<Change_Num_Otp> {
  final Color _appBarColor = Colors.lightBlueAccent;

  @override
  Widget build(BuildContext context) {
    String number = widget.number;
    number = number.substring(7, 10);
    ...
    try {
      if (digit1.isNotEmpty &&
        ...
        final response = await DioClinetToken.instance.dio!
            .post('/customer/edit_phone_number', data: {
        ...
        if (response.statusCode == 200) {
          ...
          if (otpRes.data?.message == 'Success') {
            await EasyLoading.showSuccess("Number changed successfully");
            SharedPreferencesHelper.setUserPhone(widget.number);
  ...
  Future<void> resendotp() async {
    try {
      final response = await DioClinetToken.instance.dio!.post(
          ...
          data: {'phone_number': widget.number, '_method': 'PUT'});
      var data = jsonDecode(response.toString());
      if (response.statusCode == 200) {
        Otp_S_Response otpRes = Otp_S_Response.fromJson(data);
